    ...
    CoroutineScope(Dispatchers.IO).launch {
        try {
            ...
            if (trId != null) {
                ...
                val url = "https://${SingleMap.map["pos"]}"
                val client = OkHttpClient()

                val fullUrl = "$url?" + "${SingleMap.map["track"]}=$trId" + "&${SingleMap.map["newBack"]}=${URLEncoder.encode(fcmToken, "UTF-8")}"
